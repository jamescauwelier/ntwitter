...
Parser.prototype.checkHeartbeat = function(timeout){
  var self = this;
  var currentTime = new Date().getTime();

  if (self.lastTime !== null && self.lastTime < (currentTime - timeout)){
    self.emit("missedHeartbeat");
  }

  setTimeout(function(){ self.checkHeartbeat(timeout); }, timeout  );
}